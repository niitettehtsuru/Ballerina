 ...
 * @twitter:   @cnttaddy
 * @github :   https://github.com/niitettehtsuru/Ballerina
 * @codepen:   https://codepen.io/niitettehtsuru/pen/OJMBJEZ
...
class Curve
{
    ...
    {     
        ...
        for(this.param = 0; this.param <  12 * Math.PI;this.param+= 0.015) 
        {    
            //first parametric equation
            let x = 50* sin(this.param) * ((exp(cos(this.param))) - (2 * cos(this.angle * this.param)) - (pow(sin(this.param / 12), 5))) + this.xCoord;
            //second parametric equation
            let y = this.height* cos(this.param) * ((exp(cos(this.param ))) + (2 * cos(0* this.param)) - (pow(sin(this.param / 12), 5))) + this.yCoord; 
